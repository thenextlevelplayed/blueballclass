...
public class State<T>
{
    ...
    public IAction<TriggerContext<T>> EntryAction { get; set; }
    public IAction<TriggerContext<T>> ExitAction { get; set; }
...
public class FiniteStateMachine<T> : IStateMachine<T>
{
    ...
    public List<Transition<T>> Transitions { get; set; } = new List<Transition<T>>();
    ...
    private void Exit(Transition<T> t, TriggerContext<T> ctx)
    {
        ...
        if (t.To != null)
        {
            CurrentState.Exit(ctx.Context); // 👈 關鍵：傳 Context 給 State
        }
    }
    ...
    {
        var ctx = new TriggerContext(inputEvent = e, tContext = ..., signal = LifecycleSignal.None)
        return Handle(ctx);
    }

    private FsmResult ResolveAutomaticTransitions(TriggerContext<T> context)
    {
        var ctx = baseCtx.WithSignal(LifecycleSignal.Entry);
        ...
        for (int i = 0; i < MAX_ROUNDS; i++)
        {
            ...
            accumulated = Merge(accumulated, res);
        }
