...
{
    private DiagnosisHandler? Next { get; set; }

    protected DiagnosisHandler(DiagnosisHandler? next)
    {
        Next = next;
    }
    ...
    public void TemplateDiagnose(Patient patient, List<Symptom> symptoms, PatientDb patientDb,
        List<IDiagnosisStrategy> diagnosisStrategy)
    {
        if (CanHandle(patient, symptoms))
        {
            var currentCase = CaseHandle(patient, symptoms, patientDb, diagnosisStrategy);
            patientDb.AddCaseToPatient(patient, currentCase);
        }
        else if (Next != null)
        {
            Next.TemplateDiagnose(patient, symptoms, patientDb, diagnosisStrategy);
        }
    }
...
public class CovidHandler(DiagnosisHandler? next) : DiagnosisHandler(next)
{
        ...
        List<IDiagnosisStrategy> diagnosisStrategy)
    {
        // var medicines = new List<Medicine>();
        // medicines.Add(new Medicine("清冠一號"));
        return new Case(symptoms,
            diagnosisStrategy.First(m => typeof(CovidStrategy) == m.GetType()).CurrentPrescription
...
public class AttractiveHandler(DiagnosisHandler? next) : DiagnosisHandler(next)
{
        ...
        List<IDiagnosisStrategy> diagnosisStrategy)
    {
        ...
        // medicines.Add(new Medicine("臭味"));
        return new Case(symptoms,
            diagnosisStrategy.First(m => typeof(AttractiveStrategy) == m.GetType()).CurrentPrescription, new DateTime().ToLocalTime());
    }
}
...
public class SleepApneaHandler(DiagnosisHandler? next) : DiagnosisHandler(next)
{
        ...
        List<IDiagnosisStrategy> diagnosisStrategy)
    {
        ...
        // medicines.Add(new Medicine("一捲膠帶"));
        return new Case(symptoms,
            diagnosisStrategy.First(m => typeof(SleepApneaStrategy) == m.GetType()).CurrentPrescription, new DateTime().ToLocalTime());
    }
}